...
def data_generator(myTimer: func.TimerRequest) -> None:
    ...
    container = blob_service_client.get_container_client("ci-dataproject")

    client_blobs = container.list_blob_names(name_starts_with="clients/")
    if not list(client_blobs):
        clients = generate_clients(50)
        clients_payload = json.dumps(clients).encode("utf-8")
    ...
    book_blobs = container.list_blob_names(name_starts_with="books/")
    if not list(book_blobs):
        books = generate_books(100)
        books_payload = json.dumps(books).encode("utf-8")
    ...
    orders_blob = container.get_blob_client(f"orders/{orders_file_name}")
